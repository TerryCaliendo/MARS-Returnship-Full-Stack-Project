...
export default function Todo({ todo }) {
  ...
  // useEffect(() => {
  //   setLocalTodoTitle(todo.title);
  // }, [todo]);

  let editKeystrokeHandler = (title) => {
    dispatch(editTodoAction(todo.id, title));
  };
  ...
  let saveHandler = () => {
    ...
    dispatch(editTodoAction(todo.id, todo.title));
        ...
        <div className="d-flex bd-highlight">
          ...
          {editFlag ? (
            <input
              ...
              onChange={(e) => {
                editKeystrokeHandler(e.target.value);
